...
- 위 아래 2단 구조
- 위: 셀렉트 창으로 로그 타겟들을 선택 기본은 App Logs, 그 다음은 bottles 이름이다. 선호 바틀을 선택해 놓으면 셀렉트 창 위쪽에 선호 바틀의 버튼이 생성된다. 생성 시에는 애니메이션으로 부드럽게 생성되고 log target의 div가 리사이즈 되는 애니메이션이 필요함. 자연스럽게 선호 바틀의 버튼이 추가되도록 한다.
- 아래: 좌우 2단으로 분리되어 있음
